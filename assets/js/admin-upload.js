...
( function () {
	...
	function refreshGallerySelectors() {
		...
		jQuery.post(
			ajaxurl,
			{
				...
				nonce: window.egMediaUploadData
					? window.egMediaUploadData.nonce
					: '',
			},
			function ( response ) {
				if ( response.success && Array.isArray( response.data ) ) {
					...
					const uploadSelect = document.getElementById(
						'eg_media_target_gallery'
					);
					if ( uploadSelect ) {
						const currentVal = uploadSelect.value;
						const newGalleryInput = document.getElementById(
						...
						if (
							newGalleryInput &&
							newGalleryInput.value.trim() !== ''
						) {
							const createdName = newGalleryInput.value
								.trim()
								.toLowerCase();
						...
						let optionsHtml =
							'<option value="">— Aucune galerie par défaut —</option>';
					...
					const bulkSelects = document.querySelectorAll(
						'.eg-media-bulk-gallery-select'
					);
					bulkSelects.forEach( function ( select ) {
						...
						optionsHtml +=
							'<option value="orphan">— Sans affectation —</option>';
						galleries.forEach( function ( gallery ) {
							optionsHtml += `<option value="${ gallery.term_id }">${ gallery.name }</option>`;
						} );
	...
	if ( typeof wp !== 'undefined' && wp.Uploader ) {
		...
		wp.Uploader = function ( options ) {
			const gallerySelect = document.getElementById(
				'eg_media_target_gallery'
			);
			const newGalleryInput = document.getElementById(
				'eg_media_new_target_gallery'
			...
			if ( instance.uploader ) {
				instance.uploader.bind( 'BeforeUpload', function ( up ) {
					const latestSelect = document.getElementById(
						'eg_media_target_gallery'
					);
					const latestNewInput = document.getElementById(
						'eg_media_new_target_gallery'
					);

					const currentVal = latestSelect ? latestSelect.value : '';
					const currentNewVal = latestNewInput
						? latestNewInput.value
						: '';

					up.settings.multipart_params =
						up.settings.multipart_params || {};
					up.settings.multipart_params.eg_media_target_gallery =
						currentVal;
	...
	document.addEventListener( 'DOMContentLoaded', function () {
		...
		function moveSelectorToTop() {
			const container = document.querySelector(
				'.eg-media-upload-gallery-container'
			);
		...
		function updateAllUploaders() {
			...
			if (
				...
				wp.Uploader.defaults.multipart_params =
					wp.Uploader.defaults.multipart_params || {};
				wp.Uploader.defaults.multipart_params.eg_media_target_gallery =
					val;
				wp.Uploader.defaults.multipart_params.eg_media_new_target_gallery =
					newVal;
			}
			...
			if ( typeof uploader !== 'undefined' && uploader.settings ) {
				uploader.settings.multipart_params =
					uploader.settings.multipart_params || {};
				uploader.settings.multipart_params.eg_media_target_gallery =
					val;
				uploader.settings.multipart_params.eg_media_new_target_gallery =
					newVal;
			}
			...
			if (
				...
				wp.media &&
				wp.media.uploader &&
				wp.media.uploader.uploader
			) {
				const wpUp = wp.media.uploader.uploader;
				if ( wpUp.settings ) {
					wpUp.settings.multipart_params =
						wpUp.settings.multipart_params || {};
					wpUp.settings.multipart_params.eg_media_target_gallery =
						val;
					wpUp.settings.multipart_params.eg_media_new_target_gallery =
						newVal;
				}
			}
		}
		...
		function bindToGlobalUploader() {
			...
			if (
				typeof uploader !== 'undefined' &&
				uploader.bind &&
				! uploader._egMediaBound
			) {
		...
		document.addEventListener( 'change', function ( event ) {
			if (
				event.target &&
				event.target.id === 'eg_media_target_gallery'
			) {
				const newGalleryInput = document.getElementById(
					'eg_media_new_target_gallery'
				);
		...
		document.addEventListener( 'input', function ( event ) {
			if (
				event.target &&
				event.target.id === 'eg_media_new_target_gallery'
			) {
		...
		if (
			...
			wp.media.view.AttachmentFilters.EGMediaGallery =
				wp.media.view.AttachmentFilters.extend( {
					...
					createFilters() {
						...
						if (
							window.egMediaUploadData &&
							window.egMediaUploadData.galleries
						) {
							window.egMediaUploadData.galleries.forEach(
								function ( gallery ) {
			...
			wp.media.view.AttachmentsBrowser =
				OriginalAttachmentsBrowser.extend( {
					createToolbar() {
						...
						if ( this.collection && this.collection.props ) {
							...
							if (
								! filterVal &&
								urlParams.has( 'eg_media_gallery' )
							) {
								const slug =
									urlParams.get( 'eg_media_gallery' );
								if (
									window.egMediaUploadData &&
									window.egMediaUploadData.galleries
								) {
									const matched =
										window.egMediaUploadData.galleries.find(
											function ( g ) {
												return (
													g.slug === slug ||
													String( g.term_id ) === slug
												);
											}
										);
									if ( matched ) {
							...
							if ( filterVal ) {
								...
								const finalVal =
									! isNaN( parsedInt ) &&
									String( parsedInt ) === String( filterVal )
										? parsedInt
										: filterVal;
						...
						this.toolbar.set(
							...
							new wp.media.view.AttachmentFilters.EGMediaGallery(
								{
			...
			if ( wp.media.view.Attachment ) {
				...
				wp.media.view.Attachment = OriginalAttachment.extend( {
					className() {
						let classes = '';
						if (
							typeof OriginalAttachment.prototype.className ===
							'function'
						) {
							classes =
								OriginalAttachment.prototype.className.apply(
									this,
									arguments
								);
						} else if (
					...
					render() {
						if (
							typeof OriginalAttachment.prototype.render ===
							'function'
						) {
							OriginalAttachment.prototype.render.apply(
								this,
								arguments
							);
						}
						if ( this.model.get( 'eg_media_is_reference' ) ) {
							const galleryName =
								this.model.get(
									'eg_media_reference_gallery_name'
								) || '';
							const starHtml = `<div class="eg-media-star-badge" title="Image de référence de la galerie : ${ _.escape(
								galleryName
							) }">★</div>`;
				...
				if ( wp.media.view.Attachment.Library ) {
					const OriginalAttachmentLibrary =
						wp.media.view.Attachment.Library;
					wp.media.view.Attachment.Library =
						OriginalAttachmentLibrary.extend( {
							className() {
								let classes = '';
								if (
									typeof OriginalAttachmentLibrary.prototype
										.className === 'function'
								) {
									classes =
										OriginalAttachmentLibrary.prototype.className.apply(
											this,
											arguments
										);
								} else if (
									...
									classes =
										OriginalAttachmentLibrary.prototype
											.className;
								}
								if (
									this.model.get( 'eg_media_is_reference' )
								) {
									classes += ' eg-media-is-reference';
								}
								return classes;
							...
							render() {
								if (
									...
									OriginalAttachmentLibrary.prototype.render.apply(
										this,
										arguments
									);
								}
		...
		function initBulkActionsListMode() {
			const bulkSelectors = document.querySelectorAll(
			...
			let optionsHtml =
				'<option value="">' +
				translate( '— Choisir une galerie —', 'eg-media' ) +
				'</option>';
			optionsHtml +=
				'<option value="orphan">' +
				translate( '— Sans affectation —', 'eg-media' ) +
				'</option>';
			if (
				window.egMediaUploadData &&
				window.egMediaUploadData.galleries
			...
			const inputs = document.querySelectorAll(
				'.eg-media-bulk-new-gallery-input'
			);
